...
<!--
  ...
  This stylesheet can be used for converting W3C XML Schema
  into DocBook documentation. It was derived from xs3p.xsl by
  ASW Systems s.r.o. See file COPYING for license.
...
<xsl:stylesheet 
 ...
 exclude-result-prefixes="xsd ppp html">
   ...
   <!-- Number of 'em' to indent from parent element's start tag to
        attribute's tag -->
   <xsl:variable name="ATTR_INDENT">2</xsl:variable>
...
'searchIncludedSchemas' or 'searchImportedSchemas' is true.
         ...
         <section>
           ...
           <section>
            ...
            <xsl:if test="xsd:redefine">
               <section>
                 <title>Redefined Schema Components</title>
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <section>
                  <title>Global Schema Components</title>
   ...
   <xsl:template match="xsd:*[@name]" mode="topSection">
      ...
      <!-- XML Instance Representation Documentation -->
      <xsl:call-template name="SampleInstanceDocumentation">
         <xsl:with-param name="component" select="."/>
      </xsl:call-template>
   ...
   <xsl:template name="ComponentSectionHeader">
      ...
      <xsl:attribute name="id"><xsl:value-of select="$component/@name"/></xsl:attribute>
        ...
        <title>
         ...
         <xsl:text>: </xsl:text>
         <!-- Name -->
         <xsl:value-of select="$component/@name"/></title>
   </xsl:template>
   ...
   <xsl:template name="Legend">
    ...
    <section>
         ...
         <row>
            <xsl:choose>
               <xsl:when test="normalize-space(translate($printAllSuperTypes, 'TRUE', 'true'))='true'">
                  <entry><para>Super-types:</para></entry>
                  <entry><para>
                     <xsl:text>Address</xsl:text>
                     <xsl:text> &lt; </xsl:text>
                     <xsl:text>AusAddress</xsl:text>
                     <xsl:text> (by extension)</xsl:text>
                  </para></entry>
               </xsl:when>
               <xsl:otherwise>
                  <entry><para>Parent type:</para></entry>
                  <entry><para>
                     <xsl:text>Address</xsl:text>
         ...
         <row>
            <xsl:choose>
               <xsl:when test="normalize-space(translate($printAllSubTypes, 'TRUE', 'true'))='true'">
                  <entry><para>Sub-types:</para></entry>
                  <entry><para>
                     <itemizedlist>
                        <listitem><para>
                           <xsl:text>QLDAddress</xsl:text>
               ...
               <xsl:otherwise>
                  <entry><para>Direct sub-types:</para></entry>
                  <entry><para>
                     <itemizedlist>
                        <listitem><para>
                           <xsl:text>QLDAddress (by restriction)</xsl:text>
                        </para></listitem>
                     </itemizedlist>
                  </para></entry>
               </xsl:otherwise>
      ...
      <tgroup>
        <xsl:attribute name="cols">2</xsl:attribute>
         ...
         <row>
            ...
            <entry><para>AusAddress</para></entry>
         </row>
         <row>
            <entry><para>
      ...
      <div class="hint">
         ...
         <itemizedlist>
            <listitem><para>The minimum and maximum occurrence of elements and attributes are provided in square brackets, e.g. [0..1].</para></listitem>
            <listitem><para>Model group information are shown in gray, e.g. Start Choice ... End Choice.</para></listitem>
            <listitem><para>For type derivations, the elements and attributes that have been added to or changed from the base type's content are shown in <span style="font-weight: bold">bold</span>.</para></listitem>
            <listitem><para>If an element/attribute has a fixed value, the fixed value is shown in green, e.g. country="Australia".</para></listitem>
            <listitem><para>Otherwise, the type of the element/attribute is displayed.
               <itemizedlist>
                  <listitem><para>If the element/attribute's type is in the schema, a link is provided to it.</para></listitem>
                  <!--<listitem><para>An <em>E</em> symbol is shown if the element/attribute's type is located in an external schema.</para></listitem>-->
                  <listitem><para>For local simple type definitions, the constraints are displayed in angle brackets, e.g. &lt;&lt;<em>pattern</em> = [1-9][0-9]{3}>>.</para></listitem>
               </itemizedlist>
            </para></listitem>
         </itemizedlist>
      </div>
   ...
   <!-- 
     ...
     Assumes it will be called within XHTML <itemizedlist> tags.
     Param(s):
         ...
         <xsl:otherwise>
            ...
            <xsl:choose>
               ...
               <xsl:when test="normalize-space(translate($printAllSuperTypes, 'TRUE', 'true'))='true'">
                 ...
                 <xsl:call-template name="CreateTextWithLinks">
        ...
        <xsl:attribute name="cols">2</xsl:attribute>
         ...
         <xsl:if test="@default">
            <row>
               ...
               <entry><para><xsl:value-of select="@default"/></para></entry>
            </row>
         </xsl:if>
         <!-- Fixed Value -->
         <xsl:if test="@fixed">
            <row>
               <entry><para>Fixed Value</para></entry>
               <entry><para><xsl:value-of select="@fixed"/></para></entry>
            </row>
         </xsl:if>
         <!-- Annotation -->
        ...
        <xsl:attribute name="cols">2</xsl:attribute>
         ...
         <xsl:if test="normalize-space($final)!=''">
            <row>
               ...
               <entry><para><xsl:value-of select="$final"/></para></entry>
            </row>
         </xsl:if>
         <!-- Block -->
         <xsl:variable name="block">
         ...
         <xsl:if test="normalize-space($block)!=''">
            <row>
               ...
               <entry><para><xsl:value-of select="$block"/></para></entry>
            </row>
         </xsl:if>
         <!-- Annotation -->
         <xsl:call-template name="PrintAnnotation">
   ...
   <xsl:template match="xsd:notation" mode="properties">
      ...
      <tgroup>
        <xsl:attribute name="cols">2</xsl:attribute>
         ...
         <!-- Public Identifier -->
         <row>
            <entry><para>Public Identifier</para></entry>
            <entry><para><xsl:value-of select="@public"/></para></entry>
         </row>
         <!-- System Identifier -->
         <xsl:if test="@system">
            <row>
               <entry><para>System Identifier</para></entry>
               <entry><para><xsl:value-of select="@system"/></para></entry>
            </row>
         </xsl:if>
         <!-- Annotation -->
         <xsl:call-template name="PrintAnnotation">
        ...
        <xsl:attribute name="cols">2</xsl:attribute>
         ...
         <xsl:if test="@version">
            <row>
               ...
               <entry><para><xsl:value-of select="@version"/></para></entry>
            </row>
         </xsl:if>
         <!-- Language -->
         <xsl:if test="@xml:lang">
            <row>
               ...
               <entry><para><xsl:value-of select="@xml:lang"/></para></entry>
            </row>
         </xsl:if>
         <!-- Element/Attribute Form Defaults -->
         <row>
            <entry><para>Element and Attribute Namespaces</para></entry>
            <entry><para>
               <itemizedlist>
                  <listitem><para>Global element and attribute declarations belong to this schema's target namespace.</para></listitem>
                  <listitem><para>
                     <xsl:choose>
         ...
         <xsl:if test="xsd:include or xsd:import or xsd:redefine">
            <row>
               <entry><para>Schema Composition</para></entry>
               <entry><para>
                  <itemizedlist>
                     <!-- Import -->
                     ...
                     <xsl:if test="xsd:redefine">
                        ...
                        <xsl:text>See Redefined Schema Components section.</xsl:text>
                        </para></listitem>
                     </xsl:if>
                  </itemizedlist>
               </para></entry>
   ...
   <xsl:template name="PrintAnnotation">
      ...
      <xsl:if test="$component/xsd:annotation/xsd:appinfo">
         <row>
            <entry><para>Application Data</para></entry>
            <entry><para>
   ...
   <xsl:template name="SampleInstanceDocumentation">
      <xsl:param name="component"/>
      ...
      <xsl:if test="local-name($component)!='simpleType' and local-name($component)!='notation'">
         ...
         <xsl:call-template name="CollapseableBox">
            <xsl:with-param name="id" select="concat($componentID, '_xibox')"/>
            <xsl:with-param name="styleClass">sample</xsl:with-param>
            <xsl:with-param name="caption">XML Instance Documentation</xsl:with-param>
            <xsl:with-param name="contents">
               <xsl:apply-templates select="$component" mode="documentation"/>
            </xsl:with-param>
            <xsl:with-param name="isOpened">true</xsl:with-param>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template match="xsd:all" mode="documentation">
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="schemaLoc">this</xsl:param>
      ...
      <xsl:if test="normalize-space(@maxOccurs)!='0'">
         <!-- Header -->
            ...
            <!-- Documentation -->
            <xsl:call-template name="PrintSampleDocumentationDocumentation">
               <xsl:with-param name="component" select="."/>
            </xsl:call-template>
<xsl:text>
</xsl:text>
...
<xsl:text>
      ...
      <xsl:value-of select="substring('                                                                           ',1,$margin)"/>
...
-->
   ...
   <xsl:template match="xsd:attribute[@name]" mode="documentation">
      <xsl:param name="subTypeAttrs"/>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="margin">0</xsl:param>
...
<xsl:text>
   ...
   <xsl:template match="xsd:attribute[@ref]" mode="documentation">
      <xsl:param name="subTypeAttrs"/>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="margin">0</xsl:param>
   ...
   <xsl:template match="xsd:attributeGroup[@ref]" mode="documentation">
      <xsl:param name="subTypeAttrs"/>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="margin">0</xsl:param>
   ...
   <xsl:template match="xsd:choice" mode="documentation">
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="parentGroups"/>
...
<xsl:text>
   ...
   <xsl:template match="xsd:complexType" mode="documentation">
      <xsl:param name="schemaLoc">this</xsl:param>
      <xsl:call-template name="PrintSampleComplexElementDocumentation">
         <xsl:with-param name="type" select="."/>
         <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
      </xsl:call-template>
   </xsl:template>
            ...
            <xsl:choose>
               ...
               <xsl:when test="@type">
                  ...
                  <xsl:choose>
                     ...
                     <!-- Complex type was not found. -->
                     <xsl:when test="normalize-space($defLoc)='' or normalize-space($defLoc)='none' or normalize-space($defLoc)='xml' or normalize-space($defLoc)='xsd'">
                        <xsl:call-template name="PrintSampleSimpleElementDocumentation">
                           <xsl:with-param name="element" select="."/>
                           <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
                        </xsl:call-template>
                     </xsl:when>
...
<xsl:value-of select="substring('                                                                           ',1,$margin)"/>
   ...
   <xsl:template match="xsd:sequence" mode="documentation">
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      <xsl:param name="parentGroups"/>
...
<xsl:value-of select="substring('                                                                           ',1,$margin)"/>
         ...
         <xsl:apply-templates select="xsd:*" mode="documentation">
            <xsl:with-param name="margin">
               <xsl:choose>
                  <xsl:when test="normalize-space($occursInfo)='[1]'">
                     <xsl:value-of select="$margin"/>
...
<xsl:text>
  ...
  -->
   <xsl:template name="PrintSampleDocumentation">
      ...
      <xsl:if test="$component/xsd:annotation/xsd:documentation">
   <!--
        (<xsl:call-template name="RozsekejDokumentaci">
            <xsl:with-param name="documentation" select="$component/xsd:annotation/xsd:documentation"/>
         </xsl:call-template>)  -->
      </xsl:if>
      <xsl:text>
</xsl:text>
   ...
   <xsl:template name="PrintSampleDocumentationDocumentation">
      <xsl:param name="component"/>
      <xsl:if test="$component/xsd:annotation/xsd:documentation">
        <xsl:text> (</xsl:text>
          <xsl:value-of select="translate($component/xsd:annotation/xsd:documentation,'&#013;&#010;','  ')"/>
        <xsl:text>)</xsl:text>
      </xsl:if>
   </xsl:template>


   <xsl:template name="RozsekejDokumentaci">
      <xsl:param name="documentation"/>
         <xsl:value-of select="substring($documentation,1,68)"/>
      <xsl:if test="string-length($documentation)>68">
      <xsl:text>
         </xsl:text>
         <xsl:call-template name="RozsekejDokumentaci">
            <xsl:with-param name="documentation" select="substring($documentation,69,string-length($documentation)-48)"/>
         </xsl:call-template>
      </xsl:if>
   </xsl:template>
...
<xsl:text>
   ...
   <xsl:template name="PrintSampleSimpleElementDocumentation">
      <xsl:param name="element"/>
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      ...
      <xsl:variable name="elemTag">
         ...
         <xsl:choose>
            ...
            <!-- Local simple type definition is provided -->
            <xsl:when test="$element/@name and $element/xsd:simpleType">
               <xsl:apply-templates select="$element/xsd:simpleType" mode="documentation">
                  <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
               </xsl:apply-templates>
            </xsl:when>
            <xsl:otherwise>
         ...
         <!-- Identity Constraints -->
         <xsl:if test="$element/xsd:unique or $element/xsd:key or $element/xsd:keyref">
            <xsl:apply-templates select="$element/xsd:unique | $element/xsd:key | $element/xsd:keyref" mode="documentation">
               <xsl:with-param name="margin" select="number($margin) + number($ELEM_INDENT)"/>
               <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
            </xsl:apply-templates>
         </xsl:if>
...
<xsl:value-of select="substring('                                                                           ',1,$margin)"/>
   ...
   <xsl:template name="PrintSampleComplexElementDocumentation">
      <xsl:param name="type"/>
      <xsl:param name="element"/>
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:variable name="fromTopCType">
               ...
               <!-- Print attributes -->
               <xsl:call-template name="PrintSampleTypeAttrsDocumentation">
                  <xsl:with-param name="type" select="$type"/>
                  <xsl:with-param name="isInherited" select="$isInherited"/>
                  <xsl:with-param name="isNewField" select="$isNewField"/>
                  <xsl:with-param name="margin" select="number($margin) + number($ATTR_INDENT)"/>
               ...
               <!-- Get content -->
               <xsl:variable name="content">
                  <xsl:call-template name="PrintSampleTypeContentDocumentation">
                     <xsl:with-param name="type" select="$type"/>
                     <xsl:with-param name="margin" select="number($margin) + number($ELEM_INDENT)"/>
                     <xsl:with-param name="isInherited" select="$isInherited"/>
                     <xsl:with-param name="isNewField" select="$isNewField"/>
...
<xsl:value-of select="substring('                                                                           ',1,$margin)"/>
   ...
   <xsl:template name="PrintSampleTypeAttrsDocumentation">
      <xsl:param name="type"/>
      <xsl:param name="subTypeAttrs"/>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      ...
      <xsl:choose>
         ...
         <xsl:when test="$type/xsd:complexContent or $type/xsd:simpleContent">
            <xsl:choose>
               <xsl:when test="$type/xsd:complexContent/xsd:restriction">
                  <xsl:call-template name="PrintSampleDerivedTypeAttrsDocumentation">
                     <xsl:with-param name="derivationElem" select="$type/xsd:complexContent/xsd:restriction"/>
                     <xsl:with-param name="subTypeAttrs" select="$subTypeAttrs"/>
                     <xsl:with-param name="isInherited" select="$isInherited"/>
                     <xsl:with-param name="isNewField" select="$isNewField"/>
         ...
         <!-- No derivation -->
         <xsl:when test="local-name($type)='complexType'">
            <xsl:call-template name="PrintSampleAttrListDocumentation">
               <xsl:with-param name="list" select="$type"/>
               <xsl:with-param name="subTypeAttrs" select="$subTypeAttrs"/>
               <xsl:with-param name="isInherited" select="$isInherited"/>
               <xsl:with-param name="isNewField" select="$isNewField"/>
   ...
   <xsl:template name="PrintSampleDerivedTypeAttrsDocumentation">
      <xsl:param name="derivationElem"/>
      <xsl:param name="subTypeAttrs"/>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
   ...
   <xsl:template name="PrintSampleAttrListDocumentation">
      <xsl:param name="list"/>
      <xsl:param name="subTypeAttrs"/>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      ...
      <xsl:apply-templates select="$list/xsd:attribute | $list/xsd:attributeGroup | $list/xsd:anyAttribute" mode="documentation">
         <xsl:with-param name="subTypeAttrs" select="$subTypeAttrs"/>
         <xsl:with-param name="isInherited" select="$isInherited"/>
         <xsl:with-param name="isNewField" select="$isNewField"/>
         <xsl:with-param name="margin" select="$margin"/>
...
<xsl:text>
      ...
      <xsl:template name="PrintSampleTypeContentDocumentation">
      <xsl:param name="type"/>
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      ...
      <xsl:choose>
         ...
         <xsl:when test="$type/xsd:complexContent/xsd:restriction">
            ...
            <xsl:choose>
               ...
               <xsl:when test="normalize-space($defLoc)='' or normalize-space($defLoc)='none' or normalize-space($defLoc)='xml' or normalize-space($defLoc)='xsd'">
                     <xsl:text>&lt;!-- '</xsl:text>
                     <xsl:call-template name="PrintTypeRefDocumentation">
                        <xsl:with-param name="ref" select="$restriction/@base"/>
                        <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
                     </xsl:call-template>
                     <xsl:text>' super type was not found in this schema. Some elements and attributes may be missing. --></xsl:text>
            ...
            <!-- Print out content from this type -->
            <xsl:if test="$restriction/xsd:*[local-name(.)!='annotation']">
               <xsl:call-template name="PrintSampleParticleListDocumentation">
                  <xsl:with-param name="list" select="$restriction"/>
                  <xsl:with-param name="margin" select="$margin"/>
                  <xsl:with-param name="isInherited" select="$isInherited"/>
                  <xsl:with-param name="isNewField">
         ...
         <xsl:when test="$type/xsd:simpleContent/xsd:restriction">
            <!-- Print out simple type constraints-->
               <xsl:text> </xsl:text>
               <xsl:apply-templates select="$type/xsd:simpleContent" mode="documentation">
                  <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
               </xsl:apply-templates>
               <xsl:text> </xsl:text>
<xsl:text>
...
<xsl:text>
   ...
   <xsl:template name="PrintSampleParticleListDocumentation">
      <xsl:param name="list"/>
      <xsl:param name="margin">0</xsl:param>
      <xsl:param name="isInherited">false</xsl:param>
      <xsl:param name="isNewField">false</xsl:param>
      ...
      <xsl:if test="$list">
         <xsl:apply-templates select="$list/xsd:group | $list/xsd:sequence | $list/xsd:choice | $list/xsd:all | $list/xsd:element" mode="documentation">
            <xsl:with-param name="margin" select="$margin"/>
            <xsl:with-param name="isInherited" select="$isInherited"/>
            <xsl:with-param name="isNewField" select="$isNewField"/>
            <xsl:with-param name="schemaLoc" select="$schemaLoc"/>
...
<xsl:value-of select="substring('                                                                           ',1,$margin)"/>
   ...
   <xsl:template name="PrintTypeRefDocumentation">
      <xsl:param name="name"/>
      <xsl:param name="ref"/>
      <xsl:param name="schemaLoc">this</xsl:param>
   ...
   <xsl:template name="CreateTextWithLinks">
      <xsl:param name="contents"/>
      <xsl:choose>  
        <xsl:when test="contains($contents,'!!!link!!!')">
          <xsl:variable name="textafter" select="substring-after($contents,'!!!link!!!')"/>
          <xsl:value-of select="substring-before($contents,'!!!link!!!')"/>
          <link>
            <xsl:attribute name="linkend"><xsl:value-of select="substring-before($textafter,'|||link|||')"/></xsl:attribute>
            <xsl:value-of select="substring-before($textafter,'|||link|||')"/>
          </link>
          <xsl:call-template name="CreateTextWithLinks">
            <xsl:with-param name="contents">
              <xsl:value-of select="substring-after($textafter,'|||link|||')"/>
            </xsl:with-param>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
...
was not specified in the links file, <xsl:value-of select="$linksFile"/>.
   ...
   <xsl:template name="PrintEnumFacets">
      ...
      <xsl:if test="$simpleRestrict/xsd:enumeration">
         <xsl:text>value comes from list: {</xsl:text>
